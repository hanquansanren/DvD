...
class VGGPyramid(nn.Module):
    def __init__(self, train=False, pretrained=True):
        super().__init__()
        self.n_levels = 4
        source_model = models.vgg16(weights=VGG16_Weights.DEFAULT)
    ...
    def forward(self, x, quarter_resolution_only=False, eigth_resolution=False, sixteen_resolution=False):
        outputs = []
        ...
        elif sixteen_resolution:
            x_full = self.__dict__['_modules']['level_' + str(0)](x)
            outputs.append(x_full) # [512, 512]
            x_half = self.__dict__['_modules']['level_' + str(1)](x_full)
            x_quarter = self.__dict__['_modules']['level_' + str(2)](x_half)
            outputs.append(x_quarter) # [128, 128]
            x_eight = self.__dict__['_modules']['level_' + str(3)](x_quarter)
            x_sixteen = self.__dict__['_modules']['level_' + str(4)](x_eight)
            outputs.append(x_sixteen) # [32, 32]            
        else:
